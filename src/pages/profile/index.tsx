    ...
    const handleChangePassword = async () => {
      try {      
        if (!oldPassword || !newPassword) {
          setDialogState(false);
          swalPopUp("Error", "Both old and new password are required", "error");
          return;
        }
        ...
        if (confirmation.isConfirmed) {
          try {
            setLoading(true);
            const response = await axios.put(Endpoints.resetPassword, request, {
              headers: {
                Authorization: "Bearer " + localStorage.getItem("Bearer"),
              },
            });
            ...
            if (response.status === 200 || response.status === 201) {
              swalPopUp("Success", "Your Password has been changed", "success");
              router.push("/login");
            } else {
            ...
            swalPopUp("server", error?.response?.data?.message, "error");
          } finally {
            setLoading(false);
          }
        } else {
                ...
                <Grid container spacing={2}>
                  ...
                  >
                    ...
                    <Box sx={{ ml: 2, flexGrow: 1 }}>
                      ...
                      <Typography sx={{ color: "#37474F",fontWeight:500, fontSize:16 }}>
                        {userData?.name}
                      </Typography>
                    </Box>
                    <Box>
